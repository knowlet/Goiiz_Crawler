...
namespace Goiiz_Crawler
{
    class Rakuten
    {
        ...
        public string getSinglePage(string url)
        {
            ...
            return String.Format("{0},\"{1}\",\"{2}\",{3},{4},{5}\"{6}\",{7} ,{8} ", string.Join(" ", path), title, description, preferPrice, orgPrice, findClassId(path),
                content, string.Join(" ,", contentPic), string.Join(" ,", pic)) + Environment.NewLine;
        }
